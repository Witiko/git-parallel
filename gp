...
wrap() {
   if [[ -t 1 ]] && hash fmt 2>&-
   then
    fmt -u -w $(if hash tput 2>&- && [[ "`tput cols`" -lt 80 ]]
    then
      echo `tput cols`
    else
      echo 80
...
checkName() {
  ...
  if [[ "$(printf '%s' "$1" | wc -l)" -gt 0 ]] || grep -q '\s' <<<"$1"
  then
    errcat <<'EOF'
...
EOF
  ...
  then
    local LINK
    if ! LINK="`readlink .git`" || [[ ! $LINK =~ ^$GP_DIR_RE/.* ]]
    then
      errcat <<EOF
...
EOF
  ...
  do
    REPO="${REPO%/}"
    if checkName "$REPO" 2>/dev/null && [[ `ls $REPO | wc -l` = 0 ]]
    then
      rmdir $REPO &&
      git init &&
      mv -T .git $REPO &&
...
init() {
  ...
  then
    ...
    then
      ...
      if [[ -e .gitignore ]] && ! grep -q ^$GP_DIR_RE <.gitignore
      then
        printf '%s\n' $GP_DIR >>.gitignore
...
list() {
  ...
  # Perform the main routine.
  (shopt -s nullglob && for REPO in $GP_DIR/*/
  do
    REPO="${REPO%/}"
...
EOF
  ...
  local DEST; for DEST in ${DESTS[@]}
  do
    # Guard against bad input.
    if [[ -d $GP_DIR/$DEST ]] && ! $CLOBBER
    then
      errcat <<EOF
...
EOF
  ...
  if [[ ${#REPOS[@]} = 1 ]]
  then
    if ! SOURCE=`list --active 2>/dev/null`
    then
      errcat<<'EOF'
No source Git-parallel repository was specified and could not be inferred,
since no Git-parallel repository is currently active.
...
checkout() {
  ...
  if [[ ! -z "$REPO" && ! -d $GP_DIR/$REPO ]] && ! $CREATE
  then
    errcat <<EOF
...
do_cmd() {
  ...
  for REPO in ${REPOS[@]}
  do
    if ! checkout $REPO && ! $FORCE
    then
      FAILED=true
      break
    fi
    if (cd "$PREVIOUS_PWD" && git "${COMMAND[@]}")
    then :; else
